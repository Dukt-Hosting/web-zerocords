...
server.get("/", (req, res) => {
    if (!req.session.user) {
        ...
        return res.redirect(
            ...
            config.clientid +
            "&redirect_uri=https%3A%2F%2Fweb-zerocords.spicytofu.repl.co%2Fcallback&response_type=code&scope=identify%20guilds&state=" +
            req.session.state
        );
    } else {
        if (req.session.allowed) {
            user = req.session.user;
            return res.send('WOW YOU ARE POG!<br>' + JSON.stringify(user))
        } else {
            return sendError(
                res,
                403,
...
server.get("/callback", (req, res) => {
    ...
    fetch("https://discord.com/api/oauth2/token", {
        ...
        body:
            ...
            req.query.code +
            "&redirect_uri=https%3A%2F%2Fweb-zerocords.spicytofu.repl.co%2Fcallback&scope=identify%20guilds&state=" +
            req.session.state
    })
...
function sendError(res, code, message, json) {
    ...
    if (json) return res.json({ success: false, error: message });
        ...
        return res.send("Stinky poopy error" + message);
}
